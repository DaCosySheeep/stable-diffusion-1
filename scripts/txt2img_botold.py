...
def load_replacement(x, replacement="assets/rick.jpeg"):
    try:
        hwc = x.shape
        y = Image.open(replacement).convert("RGB").resize((hwc[1], hwc[0]))
...
def check_safety(x_image, replacement="assets/rick.jpeg"):
    ...
    for i in range(len(has_nsfw_concept)):
        if has_nsfw_concept[i]:
            x_checked_image[i] = load_replacement(x_checked_image[i], replacement=replacement)
...
class SDBot:
    ...
    def makeimg(self, prompt, filename, seed=None, batch_size=1, width=512, height=512, ddim_steps=50, outpath="outputs/txt2img-samples", do_safety_check=True, safety_replacement_image="assets/rick.jpeg"):
        ...
        self.outpath = outpath #"outputs/txt2img-samples"
        self.sample_path = os.path.join(self.outpath, "samples")
        self.img_path = outpath #r'C:\img'
                ...
                with self.model.ema_scope():
                    ...
                    for n in trange(self.n_iter, desc="Sampling"):
                        for prompts in tqdm(self.data, desc="data"):
                            ...
                            x_checked_image, has_nsfw_concept = check_safety(x_samples_ddim, replacement=safety_replacement_image) if do_safety_check else x_samples_ddim, False
